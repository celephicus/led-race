...
//Pins  
#define PIN_LED        A3  // R 500 ohms to DI pin for WS2812 and WS2813, for WS2813 BI pin of first LED to GND  ,  CAP 1000 uF to VCC 5v/GND,power supplie 5V 2A  
...
#define COLOR_CAR_1    Adafruit_NeoPixel.Color(255, 0, 0)
#define COLOR_CAR_2    track.Color(0, 255, 0)
#define COLOR_HILL     track.Color(0, 0, 255)
...
void draw_car1(void) {
  for(int i=0;i<=loop1;i++){track.setPixelColor(((word)dist1 % NPIXELS)+i, track.Color(0,255-i*20,0));};                   
}

void draw_car2(void) {
  for(int i=0;i<=loop2;i++){track.setPixelColor(((word)dist2 % NPIXELS)+i, track.Color(255-i*20,0,0));};            
}
...
void loop() {
    ...
    // Draw cars, handle overlap. 
    if ((millis() & 512)==(512*draworder)) draworder ^= 1;
